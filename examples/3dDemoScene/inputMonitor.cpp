...
void initGlobals(SceneObject *self);
...
TPSCameraObject *tpsCamera;
FPSCameraObject *fpsCamera;

void initDebounceMaps() {
...
auto populateInputMaps(map<int, bool> &inputMap, map<GameInput, bool> &inputGIMap) {
...
void ready([[maybe_unused]] SceneObject *self) {
    ...
    initDebounceMaps();
    // Should not handle controller connections here - remove when InputController ready
    if (numJoySticks < 1) {
    ...
    tpsCamera = currentGame->getCamera<TPSCameraObject>("tpsCamera");
    fpsCamera = currentGame->getCamera<FPSCameraObject>("fpsCamera");
    currentGame->setActiveCamera("tpsCamera");
}

void process(SceneObject *self) {
    if (!tpsCamera || !fpsCamera) {
        printf("InputMonitor::process: TPS or FPS camera missing! Checking again.\n");
        // Attempt to retrieve again and skip this update
        tpsCamera = currentGame->getCamera<TPSCameraObject>("tpsCamera");
        fpsCamera = currentGame->getCamera<FPSCameraObject>("fpsCamera");
        currentGame->setActiveCamera("tpsCamera");
        return;
    ...
    auto charAngle = self->getRotation();
